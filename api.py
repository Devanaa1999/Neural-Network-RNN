...
# Create Gradio interface for file and text input
def gradio_interface(text, file_input=None):
    # If file_input is provided, read the content of the file
    if file_input:
        with open(file_input.name, 'r', encoding='utf-8') as file:
            text += "\n" + file.read()  # Append the file text to the provided input text
    
    # Predict sentiment from the input text (combining text and file content if any)
    ...
    return f"Sentiment: {sentiment}\n\n{confidence}", output_file  # Return sentiment analysis and file

# Configure the interface with more examples
iface = gr.Interface(
    fn=gradio_interface,
    inputs=[
        gr.Textbox(lines=3, placeholder="Enter text here..."),  # Direct text input
        gr.File(label="Upload a text file")  # File input (optional)
    ],
    outputs=[
        "text",  # Output text for sentiment analysis
        gr.File(label="Download output file")  # Output file for result download
    ],
    title="Sentiment Analysis",
    description="Analyze the sentiment of the input text (Positive, Neutral, Negative) and download the result.",
    examples=[
